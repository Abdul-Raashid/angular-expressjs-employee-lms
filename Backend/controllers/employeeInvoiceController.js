...
const Invoice = require('../models/EmployeeInvoice'); // Import the Invoice model
const generateInvoicePDF = require('../utils/generateEmployeeInvoicePDF');
const path = require('path');
const fs = require('fs');
const os = require('os'); // Import os module to get temporary directory

// Get all invoices or generate new ones if not already present
...
exports.downloadInvoicePDF = async (req, res) => {
  try {
    ...
    const filePath = invoice.pdfFilePath;
    ...
    if (fs.existsSync(filePath)) {
      ...
      res.status(404).json({ message: 'Invoice PDF file not found on server.' });
    }
  } catch (error) {
